...
def main(data_dir):
    ...
    show_world_frame = pangolin.VarBool('ui.WorldFrame', value=True, toggle=True)
    show_trajectory = pangolin.VarBool('ui.trajectory', value=True, toggle=True)
    show_current_pose = pangolin.VarBool('ui.curr_pose', value=True, toggle=True)
    ...
    while not pangolin.ShouldQuit():
        ...
        if show_world_frame.Get():
            draw_coordinate_frame(np.eye(4), length=4.0, line_width=3)
        ...
        txt_image = create_text_image(
            frame_idx, frame_size, odometry_data
        )
